Create Codex pet row `waving` for `cool-dog`: exactly 4 full-body frames in one horizontal strip on flat pure green #00FF00.

Use the attached canonical base for identity and the layout guide only for spacing. Same pet in every frame: Use the provided image as the visual source: a cute white fluffy plush puppy, rounded teddy-like body, tiny upright ears, soft shaggy fur, black bead eyes, black button nose, round front paws with dark paw pads, friendly cool-dog personality. Keep it as a compact whole-body Codex pet.. Preserve silhouette, face, palette, material, proportions, markings, and props.
