...
alter table drive_items
  ...
  add column if not exists parent_id bigint references drive_items(id) on delete cascade,
...
begin
  if not exists (
    select 1 from drive_items
    where owner_id is null or kind is null or name is null
  ) then
    alter table drive_items
...
begin
  if not exists (
    select 1
    from pg_constraint
    where conrelid = 'drive_items'::regclass
      and conname = 'drive_items_kind_check'
  ) then
    alter table drive_items
      add constraint drive_items_kind_check check (kind in ('FOLDER', 'FILE')) not valid;
  end if;
end $$;
...
begin
  if not exists (
    ...
    from pg_constraint
    where conrelid = 'drive_items'::regclass
      and conname = 'drive_items_name_length_check'
  ) then
    alter table drive_items
      add constraint drive_items_name_length_check check (length(btrim(name)) between 1 and 180) not valid;
  end if;
end $$;
...
begin
  if not exists (
    ...
    from pg_constraint
    where conrelid = 'drive_items'::regclass
      and conname = 'drive_items_file_payload_check'
  ) then
    alter table drive_items
      add constraint drive_items_file_payload_check check (
        (kind = 'FILE' and object_key is not null and size_bytes >= 0)
        or (kind = 'FOLDER' and object_key is null and size_bytes = 0)
      ) not valid;
  end if;
...
begin
  if not exists (
    ...
    from pg_constraint
    where conrelid = 'drive_items'::regclass
      and conname = 'drive_items_no_self_parent_check'
  ) then
    alter table drive_items
      add constraint drive_items_no_self_parent_check check (parent_id is null or parent_id <> id) not valid;
  end if;
end $$;
